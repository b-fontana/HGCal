...
/*
  void
  HGCalRecHitsMask::endRun(edm::Run const&, edm::EventSetup const&)
  {
  }
*/
...
/*
  void
  HGCalRecHitsMask::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
  {
  }
*/
...
/*
  void
  HGCalRecHitsMask::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
  {
  }
*/

bool HGCalRecHitsMask::cellFilter(const HGCSiliconDetId &s) {
  if(!gHGCal_) 
...
void HGCalRecHitsMask::createHistograms() {
  if(!gHGCal_) 
    throw std::invalid_argument("No geometry was selected.");
...
void HGCalRecHitsMask::fillGeomHistograms(int_layer layer, int wId, std::pair<int,int> cUV) {
  //if the histogram already exists
  if (histosGeom_[layer].count(wId))
    histosGeom_[layer][wId]->Fill(cUV.first, cUV.second);	
}

void HGCalRecHitsMask::perpHistogram(GlobalPoint p) {
