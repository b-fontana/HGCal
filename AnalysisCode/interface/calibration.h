...
void check_vector_is_sorted(const vec1d<double_>&);
  ...
  vec1d<float_> enreg;
  vec1d<float_> phireg;
  vec2d<float_> phireg_fineeta;
  vec2d<float_> etareg_fineeta;
  vec2d<float_> enreg_fineeta;
...
class Calibrator
{
 private:
  ...
  void draw_spectrum(TH2D*, TGraphAsymmErrors*, TSpline3*, const std::string&, const std::string&);
  vec3d<float> get_values_for_calibration(const std::string&, const std::string&, const std::string&,
					  const umap< CalibrationType, std::pair<float_,float_> >&,
					  const CalibrationType& calib_type, opt<vec1d<float_>> calib_var_opt = std::nullopt) const;
  ...
  bool is_outside_calibration_window(const umap< CalibrationType, std::pair<float_,float_> >&,
				     const umap< CalibrationType, float_> &) const;
 ...
 public:
  ...
  ~Calibrator();
  void create_pion_calibration_values(const vec1d<int_>&, const std::vector<std::string>&,
				      const bool_&, const bool_&,
				      opt<vec1d<CalibrationType>> calib_types_opt=std::nullopt,
				      opt<vec3d<float_>> calib_vars_opt=std::nullopt);
  void create_photon_calibration_values(const vec1d<int_>&, const std::vector<std::string>&,
					const bool_&, const bool_&,
