...
def RMS_Bias(f):
    ...
    with RootPlotting(ncanvas=1, npads=len(hn)*NREG, 
        ...
        lc = [4, 2, 3, 7]
        legends1 = [TLegend(0.12, 0.12, 0.35, 0.32) for  _ in range(3)]
        legends2 = [TLegend(0.12, 0.69, 0.35, 0.89) for  _ in range(3)]
        for ireg in range(NREG):
            for ih in range(len(f)):
                ...
                plot.plotHistogram(cpos=0, ppos=ireg, 
                                   ...
                                   h=hrms[ih*NREG+ireg], yranges=(-0.5,0.3), 
                                   draw_options=opt)
                legends1[ireg].AddEntry(hrms[ih*NREG+ireg], 'Mask '+str(ih+3), 'L')
                legends1[ireg].Draw()
                plot.plotHistogram(cpos=0, ppos=ireg+3, 
                                   lc=lc[ih], mc=lc[ih], title='RMS/(1+Bias) vs eta', 
                                   h=hindep[ih*NREG+ireg], yranges=(-.2, 2.), 
                                   draw_options=opt)
...
def Resolution(f):
    ...
    hres_complete_bef =   [f[i].Get(hn[0].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
    hres_complete_aft =   [f[i].Get(hn[1].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
    hres_incomplete_bef = [f[i].Get(hn[2].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
    hres_incomplete_aft = [f[i].Get(hn[3].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
    hres_eta_bef =        [f[i].Get(hn[4].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
    hres_eta_aft =        [f[i].Get(hn[5].format(ireg)) 
                           for i in range(len(f)) for ireg in range(1,NREG+1)]
