...
{
 public:
 SlimmedHit() : layerId_(0), assocROIidx_(-1), subdet_(1), signalRegion_(-1), en_(0), en_mip_(0), t_(0), cellSize_(0) {}
 SlimmedHit(int layerId, int assocROIidx, int subdet, int signalRegion, float en, float en_mip, float t, float cellSize) :
  layerId_(layerId), assocROIidx_(assocROIidx), subdet_(subdet), signalRegion_(signalRegion), en_(en), en_mip_(en_mip), t_(t), cellSize_(cellSize) 
  {
  }
  ...
  int   assocROIidx_, subdet_, signalRegion_;
